...
def get_students():
    ...
    # sys.exit()
    # global cursor_students
    
    cursor_students = students_coll.find()
    cursor_1 = json.loads(dumps(cursor_students))
...
def get_student_classes(student_id):
	try:
	    # global cursor_students
	    cursor_students = students_coll.find({ '_id': int(student_id) })
	    ...
	    cursor_grades = grades_coll.find({ 'student_id': int(student_id) })
		...
		return 'Please provide correct student id.!'
...
def get_classes():
    ...
    cursor = grades_coll.distinct( "class_id" )
    cursor_1 = json.loads(dumps(cursor))
    print(cursor_1)
    classes=[]
    for class_ in cursor_1:
        temp = {}
        temp["class_id"] = str(class_)
        classes.append(temp)
...
def get_class_students(class_id):
	try:
	    ...
	    cursor_grades = grades_coll.find({ 'class_id': int(class_id) })
	    cursor_grades_1 = json.loads(dumps(cursor_grades))
	    grades_df = json_normalize(cursor_grades_1)
	    # print(grades_df.head())
	    # print(class_id)
		...
		return 'Please provide correct class id.!'
...
# Get grades for a particular course type 1
@app.route('/class/<class_id>/student/<student_id>', methods=['GET'])
# Get grades for a particular course type 2
...
def get_class_student_final_grade_sheet(class_id, student_id):
	try:
		...
		for marks_ in range(4):
			...
			total_marks += int(list_temp[marks_]["score"])
			# print(total_marks)

		temp_1={}
		temp_1["type"] = 'total'
		temp_1["marks"] = int(total_marks)
		marks.append(temp_1)
		# student_name = cursor_students_json[int(student_id)]
		final_result = []
		final_dict = {}
		...
		print(str(e))
		return 'Please provide correct student id and class id.!'
